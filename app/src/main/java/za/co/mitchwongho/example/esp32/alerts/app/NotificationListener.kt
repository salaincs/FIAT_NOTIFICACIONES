...
class NotificationListener : NotificationListenerService() {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification) {
        // --- LÓGICA ESPECIAL PARA GOOGLE MAPS ---
        val isMapsEnabled = MainApplication.sharedPrefs.getBoolean("pref_maps_nav", false)
        if (isMapsEnabled && sbn.packageName == "com.google.android.apps.maps") {
            val bundle = sbn.notification.extras
            val instruction = bundle.getCharSequence("android.title")?.toString() ?: ""
            val distance = bundle.getCharSequence("android.text")?.toString() ?: ""

            // Solo enviamos si hay datos reales
            if (instruction.isNotEmpty()) {
                val intent = Intent(EXTRA_ACTION)
                intent.putExtra(EXTRA_APP_NAME, "MAPS") // Usamos MAPS como marca
                intent.putExtra(EXTRA_TITLE, instruction)
                intent.putExtra(EXTRA_BODY, distance)
                intent.putExtra(EXTRA_NOTIFICATION_DISMISSED, false)
                LocalBroadcastManager.getInstance(this).sendBroadcast(intent)
            }
            return // IMPORTANTE: Salimos para no procesar Maps como una app normal
        }
        // --- FIN LÓGICA MAPS ---
